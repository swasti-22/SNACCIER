...
import { isCharusatEmail, CHARUSAT_EMAIL_ERROR } from "./validators";
...
export const AuthProvider = ({ children }) => {
  ...
  const loginStudent = async (email, password) => {
    ...
    if (!isCharusatEmail(email)) {
      setLoading(false);
      throw new Error(CHARUSAT_EMAIL_ERROR);
    }
    if (isFirebaseConfigured && auth) {
      try {
  ...
  const resetStudentPassword = async (email, newPassword) => {
    ...
    try {
      ...
      if (idx > -1) {
        ...
        studentName = users[idx].name || studentName;
      } else {
        users.push({
          id: `user_${Date.now()}`,
          name: studentName,
      ...
      console.warn("Local storage password update:", e);
    }
  ...
  return (
    <AuthContext.Provider value={{ user, loading, loginStudent, signupStudent, resetStudentPassword, loginShop, logout }}>
      {children}
