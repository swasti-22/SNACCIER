...
export const preloadData = () => {
  ...
  try {
    ...
    // reset if the Yogi 99 shop or new menu items are missing to migrate cleanly
    if (!existing || !existing.includes("YOGI 99") || !existingMenuItems || !existingMenuItems.includes("yogi_1")) {
      safeSetItem("snaccier_shops", JSON.stringify(DEFAULT_SHOPS));
      safeSetItem("snaccier_menu_items", JSON.stringify(DEFAULT_MENU_ITEMS));
      safeRemoveItem("snaccier_cart");
      safeRemoveItem("snaccier_orders");
      safeRemoveItem("snaccier_rewards");
      safeRemoveItem("snaccier_notifications");
    }
    ...
    if (!localStorage.getItem("snaccier_opinions")) {
      safeSetItem("snaccier_opinions", JSON.stringify(DEFAULT_OPINIONS));
    }
  } catch (e) {
...
export const getShopById = async (shopId) => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      if (shopSnap && shopSnap.exists()) {
        ...
        // Fetch menu items with a timeout guard
        const menuCol = collection(db, 'menu_items');
        const q = query(menuCol, where('shop_id', '==', shopId), orderBy('name'));
        const menuSnap = await withTimeout(getDocs(q), 2000, null);
        const menu = menuSnap ? menuSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })) : [];
  ...
  if (isClient) {
    ...
    if (shop) {
      ...
      const menu = menuItems.filter(item => item.shop_id === shopId);
      return { ...shop, menu };
    }
  }
